...
import { Video, Videos, VideoComments, VideoComment, YouTubeChannels } from "./db/schema";
...
async function fetchAllVideosForChannel(channelId: string): Promise<string[] | null> {
  ...
  do {
    try{
      ...
      const videoIds = 
        ...
        nextPageToken = data.nextPageToken !== null ? data.nextPageToken : undefined;
...
export async function scrapeVideos() {
    ...
    if (!videoIds) {
      console.error(`No videos found for channel ${channel.channelId}`);
      continue;
    }
